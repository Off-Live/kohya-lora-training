    ...
    with torch.no_grad():
        with accelerator.autocast():
            for i, prompt in enumerate(prompts):
                ...
                print("log with :",args.log_with)

                tensor_tracker = accelerator.get_tracker("tensorboard");
                np_images = np.stack([np.asarray(img) for img in images])
                tensor_tracker.writer.add_images("validation",np_images,steps,dataformats="NHWC")
